...
public class NewRelicMessageProcessor extends MessageProcessor implements Closeable {
    ...
    public ModuleRegistrationResponse processRegistrationRequest(ModuleRegistrationRequest request) {
        ...
        Permissions permissions = new Permissions()
                ...
                .setUserIdentities(Arrays.asList(new UserIdentityPermission(UserIdentity.Type.EMAIL, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.OTHER, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.FACEBOOK, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.GOOGLE, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.MICROSOFT, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.OTHER2, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.OTHER3, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.OTHER4, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.TWITTER, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.YAHOO, Identity.Encoding.RAW), new UserIdentityPermission(UserIdentity.Type.CUSTOMER, Identity.Encoding.RAW)))
                .setDeviceIdentities(Arrays.asList(new DeviceIdentityPermission(DeviceIdentity.Type.GOOGLE_CLOUD_MESSAGING_TOKEN, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.APPLE_PUSH_NOTIFICATION_TOKEN, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.IOS_VENDOR_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.ANDROID_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.GOOGLE_ADVERTISING_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.IOS_ADVERTISING_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.APPLE_PUSH_NOTIFICATION_TOKEN, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.ROKU_ADVERTISING_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.MICROSOFT_ADVERTISING_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.MICROSOFT_PUBLISHER_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.FIRE_ADVERTISING_ID, Identity.Encoding.RAW), new DeviceIdentityPermission(DeviceIdentity.Type.ROKU_PUBLISHER_ID, Identity.Encoding.RAW)));
        ...
        processorSettings.add(new TextSetting(RpmId, "Account ID").setIsRequired(true)
                .setIsConfidential(false)
                .setDescription("New Relic Account ID"));
        processorSettings.add(new TextSetting(AppId, "Application ID").setIsRequired(false)
                .setIsConfidential(false)
                .setDescription("New Relic Application ID added to the Insights Event. Integer value of the Application ID or blank."));
        processorSettings.add(new TextSetting(InsightsInsertKey, "Insights Insert Key").setIsRequired(true)
                .setIsConfidential(true)
                .setDescription("New Relic insights Insert Key"));
        processorSettings.add(new TextSetting(AccountRegion, "Account Region").setIsRequired(true)
                .setIsConfidential(false)
                .setDefaultValue("US")
        ...
        List<Event.Type> supportedEventTypes = Arrays.asList(Event.Type.APPLICATION_STATE_TRANSITION, Event.Type.ATTRIBUTION, Event.Type.CUSTOM_EVENT, Event.Type.ERROR, Event.Type.IMPRESSION,
                // Event.Type.PRIVACY_SETTING_CHANGE,
                Event.Type.PRODUCT_ACTION, Event.Type.PROMOTION_ACTION,
                //				Event.Type.PUSH_MESSAGE_OPEN,
                //				Event.Type.PUSH_SUBSCRIPTION,
                //				Event.Type.PUSH_MESSAGE_RECEIPT,
                Event.Type.SCREEN_VIEW, Event.Type.SESSION_END, Event.Type.SESSION_START, Event.Type.USER_ATTRIBUTE_CHANGE, Event.Type.USER_IDENTITY_CHANGE);

        // this extension supports event data coming from all devices
        List<RuntimeEnvironment.Type> environments = Arrays.asList(RuntimeEnvironment.Type.ALEXA, RuntimeEnvironment.Type.ANDROID, RuntimeEnvironment.Type.FIRETV, RuntimeEnvironment.Type.IOS, RuntimeEnvironment.Type.MOBILEWEB, RuntimeEnvironment.Type.ROKU, RuntimeEnvironment.Type.SMARTTV, RuntimeEnvironment.Type.TVOS, RuntimeEnvironment.Type.UNKNOWN, RuntimeEnvironment.Type.XBOX);
        ...
        return new ModuleRegistrationResponse(CompanyName, "1.0").setDescription("<a href=\"http://newrelic.com\" target=\"_blank\">New Relic</a> Real-time insights for the modern enterprise")

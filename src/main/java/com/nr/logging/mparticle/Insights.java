...
import com.nr.api.insights.InsightsWriter;
...
public class Insights implements Closeable {
    ...
    private static final InsightsWriter insightsWriter = new InsightsWriter();
    private static final Logger log = new Logger(Insights.class);
    private static final ObjectMapper objectMapper = new ObjectMapper();
    private static final HashSet<String> reservedWords = new HashSet<>(Arrays.asList("ago", "and", "as", "auto", "begin", "begintime", "compare", "day", "days", "end", "endtime", "explain", "facet", "from", "hour", "hours", "in", "is", "like", "limit", "minute", "minutes", "month", "months", "not", "null", "offset", "or", "raw", "second", "seconds", "select", "since", "timeseries", "until", "week", "weeks", "where", "with"));
    ...
    private void account(Account account) {
        this.rpmId = account.getIntegerSetting(NewRelicMessageProcessor.RpmId, true, null);
        this.appId = account.getIntegerSetting(NewRelicMessageProcessor.AppId, false, null);
        this.insertKey = account.getStringSetting(NewRelicMessageProcessor.InsightsInsertKey, true, null);
        this.region = account.getStringSetting(NewRelicMessageProcessor.AccountRegion, false, "US");
    }
    ...
    private Map<String, Object> clean(Map<String, Object> flatMap) {
        ...
        if (flatMap.size() > 255) log.warning("clean: event contains more than 255 attributes and will be truncated upon ingestion. Attribute count: %d", flatMap.size());
        ...
        if (!cleanEventType.equalsIgnoreCase(eventType)) {
            log.warning("clean: invalid eventType: %s using: %s", eventType, cleanEventType);
            flatMap.put("eventType", cleanEventType);
        }
        ...
        for (String key : flatMap.keySet()) {
            ...
            if (value instanceof Double) value = ((Double) value).floatValue();
            else if (value instanceof Long) value = ((Long) value).floatValue();
            cleanMap.put(cleanKey, value);
        }
        return cleanMap;
    }
    ...
    private String cleanKey(String key) {
        ...
        if (key.length() > 255) {
            result = key.substring(0, 254);
            log.warning("cleanKey: truncating key to 255 characters. Original: %s Cleaned: %s", key, result);
            return result;
        }

        if (key.equalsIgnoreCase("timestamp_ms")) return "timestamp";
    ...
    public void close() throws IOException {
        ...
        try {
            log.finest("close: json: %s", objectMapper.writerWithDefaultPrettyPrinter()
                    .writeValueAsString(events));
            byte[] json = Gzip.compress(objectMapper.writeValueAsString(events));
            if (json.length > 1000000) {
                log.severe("close: compressed data > 1M. Actual length: %d", json.length);
            }

            StringBuffer result = insightsWriter.post(getURL(this.region), this.insertKey, json);
            log.fine("close: insightsWriter result: %s", result.toString());
    ...
    private String getURL(String region) {
        ...
        if (region.equalsIgnoreCase("EU")) host = (String) Config.getValue(Config.EUInsightsEndpoint);
        return String.format("https://%s/v1/accounts/%s/events", host, this.rpmId);

    }

    public void mParticleToInsights(Event event) {
        log.fine("mParticleToInsights: event %s", event);
        ...
        flatMap.put("appId", this.appId);
        flatMap.put("eventType", event.getType()
                .toString());
        flatMap.put("mpid", event.getRequest()
                .getMpId());
